    ...
    return (
      <div className="card-glass rounded-2xl p-4 border border-m-border">
        <div className="flex items-center gap-3 mb-2">
          ...
          <span className="text-sm font-body font-medium text-m-text">Supabase Sync</span>
        </div>
        <p className="text-xs font-body text-m-muted">
          <span className="text-[var(--accent)] font-mono">VITE_SUPABASE_URL</span> dan <span className="text-[var(--accent)] font-mono">VITE_SUPABASE_ANON_KEY</span> belum diisi
        </p>
      </div>
    )
  }
      ...
      {loggedIn ? (
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-white/60 border border-m-border">
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-[var(--accent)] animate-pulse-dot' : 'bg-m-coral'}`}
          ...
          <div className="grid grid-cols-2 gap-2">
            <button onClick={handlePush}
              ...
              className="flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border border-[var(--accent-glow)] bg-[var(--accent-soft)] hover:bg-[var(--accent-soft)] hover:border-[var(--accent)] transition-all disabled:opacity-40 text-[var(--accent)]">
              {pushState === 'running'
          ...
          <p className="text-[10px] font-body text-m-muted text-center leading-relaxed">
            Di HP baru → tap <span className="text-m-yellow">Ambil data dari Cloud</span><br/>
            Setelah edit di laptop → tap <span className="text-[var(--accent)]">Kirim semua ke Cloud</span>
          </p>

          <button onClick={logout}
            className="w-full flex items-center justify-center gap-2 py-2 text-xs font-body text-m-muted hover:text-m-coral border border-m-border hover:border-red-200 rounded-lg transition-all">
        ...
        <div className="space-y-3">
          <p className="text-xs font-body text-m-sub leading-relaxed">
            Login untuk menyinkronkan data antar device lewat Supabase.
          </p>
          <button onClick={login} disabled={loading || !isOnline}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-body text-sm font-semibold transition-all disabled:opacity-50 bg-m-yellow text-white hover:shadow-card-md active:scale-95">
            {loading
